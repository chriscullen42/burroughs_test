...
const nextBonus = (date) => {
    let nextBonus = moment(date).add(1,"month").date(15).startOf("day").toDate()
...
const iterateMonths = function *(dateFrom, months) {
    let currentMonth = moment(dateFrom).startOf("month").toDate()
    for(let month=0; month<months;month++) {
        let dates = {
            current: formatMonth(currentMonth),
            payDay: formatDate(nextPay(currentMonth)),
            bonusDay: formatDate(nextBonus(currentMonth))
        }
        yield dates
